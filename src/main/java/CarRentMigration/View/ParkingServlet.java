    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            if (parserType != null) {
                switch (parserType) {
                    case "dom": {
                        request.setAttribute("parkings", parkingService.getAllParkingsFromPaser(ParserType.DOM));
                        request.setAttribute("parsertype", "dom");
                        break;
                    }
                    case "sax": {
                        request.setAttribute("parkings", parkingService.getAllParkingsFromPaser(ParserType.SAX));
                        request.setAttribute("parsertype", "sax");
                        break;
                    }
                    case "stax": {
                        request.setAttribute("parkings", parkingService.getAllParkingsFromPaser(ParserType.STAX));
        ...
        getServletContext().getRequestDispatcher("/parkings").forward(request, response);
    }
